...
void JMP_ZONE_TEXT __attribute__((noreturn)) do_mmu_mapping_and_jump(void)
{
    ...
    // Switch to the dedicated JMP/payload boot stack via the ESP-IDF SET_STACK
    // helper — the canonical way to abandon a windowed-ABI stack and land on a
    // fresh one with coherent window state. It keeps the CURRENT window
    // (windowbase untouched, so the registers the code is actually running in
    // stay put — a naive `wsr windowbase 0` instead re-maps a0-a15 onto
    // different physical ARs and corrupts the frame), marks only the current
    // window valid in windowstart, pre-writes the base save area so window
    // underflow can walk back to the new SP, zeroes a0 (do_mmu never returns),
    // and flips PS.WOE around the switch so no windowed instruction runs
    // mid-switch.
    //
    // This MUST be the first thing the function does: the compiler addresses
    // the frame (locals, ctx structs, staged strings) relative to a1 and caches
    // a1-derived bases in registers (e.g. a7 = a1 + 0x100) that it reuses after
    // calls. Any such base computed BEFORE the switch points into the abandoned
    // app stack and reads/writes garbage after it (observed: the MWDT0 ctx was
    // written via the stale pre-switch a7 but handed to the HAL via a
    // post-switch a1-derived pointer → StoreProhibited inside the ROM HAL).
    uint32_t sp_top = (uint32_t)(jump_stack + sizeof(jump_stack)) - SAVE_AREA_OFFSET;
    SET_STACK(sp_top);

    // DIAG: bisect the hang. NOTE: no ESP_LOGI past `rsil` — interrupts are
    // masked so newlib lock_acquire_generic sees an ISR context and aborts.
    // esp_rom_printf is ROM code (lock-free) and works from the zone, but its
    // format strings live in flash rodata — usable only until the MMU remap.
    // RAM progress markers survive a reboot for app_main to read. TODO remove.
    DIAG_PROGRESS(1);
    esp_rom_printf("JMP0 entered\r\n");
    esp_rom_output_tx_wait_idle(0);

    // DIAG: prove the zone is entered (now on jump_stack) and that a call out
    // of the zone works before we disable the WDTs. TODO remove.
    char m_jm0[] = "JM0 entered\r\n";
    bootloader_uart0_print(m_jm0);

    // DIAG: after the stack switch. TODO remove.
    DIAG_PROGRESS(2);
    esp_rom_printf("JMP1 on jump_stack\r\n");
    esp_rom_output_tx_wait_idle(0);
    char m_jm1[] = "JM1 on jump_stack\r\n";
    bootloader_uart0_print(m_jm1);
    ...
    // DIAG TODO remove.
    DIAG_PROGRESS(4);
    esp_rom_printf("JMP3 writeback done\r\n");
    esp_rom_output_tx_wait_idle(0);

    char m_copy[] = "JMP[3] copying direct segments\r\n";
    bootloader_uart0_print(m_copy);

    // Copy direct segments into internal SRAM. Internal SRAM is uncached on the
    // S3, so plain stores are coherent — no write-back drain needed. The
    // payload's IRAM segments (0x4037xxxx) are written through their DRAM
    // aliases (0x3FC8xxxx): the S3's IRAM0 port only supports 32-bit data
    // access, so byte stores would raise a LoadStoreError (double-exception
    // with the fault handler itself faulting).
    for (int i = 0; i < safe_copy_count; i++) {
        ...
        if (dest_addr >= 0x40370000 && dest_addr < 0x403E0000) {
            // IRAM0 space (0x40370000-0x403E0000). Must use 32-bit word stores (s32i)
            ...
            // Internal DRAM (0x3FC88000+) or RTC (0x50000000+)
            uint8_t *d = (uint8_t *)dest_addr;
            for (uint32_t j = 0; j < len; j++) d[j] = src[j];
        }
    }

    // DIAG TODO remove. Stage RAM-resident format strings at DIAG_STAGE (safe
    // from the payload copies and the zone stack) BEFORE the MMU remap, so
    // every post-copy/post-MMU progress stays visible on the ROM console. (No
    // flash-rodata reads — they intermittently stall esp_rom_printf after the
    // copies and are dead post-MMU — and no flash-code calls like memcpy may
    // run after the remap.)
    char *stage = (char *)DIAG_STAGE;
    const char f_copy_done[] = "JMP4b copies done\r\n";
    const char f_p1[]        = "P1\r\n";
    const char f_p2[]        = "P2\r\n";
    const char f_p3[]        = "P3 maps=%d copies=%d\r\n";
    const char f_mn[]        = "M%d n=%lu\r\n";
    const char f_p4[]        = "P4\r\n";
    const char f_mmu_done[]  = "JMP5 MMU programmed p6\r\n";
    const char f_go[]        = "JMP6 caches flushed, jumping\r\n";
    const char f_wdt0[]      = "WDTCFG0=%08x WDTCFG1=%08x\r\n";
    const char f_wdt1[]      = "WDTCFG4=%08x WDTFEED=%08x\r\n";
    const char f_entry[]     = "ENTRY@ %08x: %08x %08x %08x %08x\r\n";
    const char f_lit[]       = "LIT@ 403745E0: %08x %08x %08x %08x\r\n";
    const char f_ban[]       = "BANNER@ 403805F4: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\r\n";
    memcpy(stage + 0,   f_copy_done, sizeof(f_copy_done));
    memcpy(stage + 32,  f_p1,        sizeof(f_p1));
    memcpy(stage + 64,  f_p2,        sizeof(f_p2));
    memcpy(stage + 96,  f_p3,        sizeof(f_p3));
    memcpy(stage + 128, f_mn,        sizeof(f_mn));
    memcpy(stage + 160, f_p4,        sizeof(f_p4));
    memcpy(stage + 192, f_mmu_done,  sizeof(f_mmu_done));
    memcpy(stage + 224, f_go,        sizeof(f_go));
    memcpy(stage + 256, f_wdt0,      sizeof(f_wdt0));
    memcpy(stage + 320, f_wdt1,      sizeof(f_wdt1));
    memcpy(stage + 384, f_entry,     sizeof(f_entry));
    memcpy(stage + 448, f_lit,       sizeof(f_lit));
    memcpy(stage + 512, f_ban,       sizeof(f_ban));
    const char *rom_copy_done = stage + 0;
    const char *rom_p1        = stage + 32;
    const char *rom_p2        = stage + 64;
    const char *rom_p3        = stage + 96;
    const char *rom_mn        = stage + 128;
    const char *rom_p4        = stage + 160;
    const char *rom_mmu_done  = stage + 192;
    const char *rom_go        = stage + 224;
    const char *rom_wdt0      = stage + 256;
    const char *rom_wdt1      = stage + 320;
    DIAG_PROGRESS(0x0C);

    // DIAG TODO remove.
    DIAG_PROGRESS(5);
    esp_rom_printf(rom_copy_done);
    esp_rom_printf(rom_p1);

    // DIAG: manual UART0 TX-drain poll (bounded) instead of the ROM
    // wait_idle, to see whether the FIFO actually drains after the copies.
    // TODO remove.
    volatile uint32_t *u0s = (volatile uint32_t *)0x6000001C;
    uint32_t fifo0 = (*u0s >> 16) & 0xFF;
    uint32_t spin = 0;
    while ((((*u0s) >> 16) & 0xFF) != 0 && spin < 1000000) spin++;
    uint32_t fifo1 = (*u0s >> 16) & 0xFF;
    esp_rom_printf(rom_p2);
    if (spin < 1000000) DIAG_PROGRESS(0x0E);      /* drained */
    else if (fifo0 == fifo1) DIAG_PROGRESS(0x0F); /* stuck, unchanged */
    else DIAG_PROGRESS(0x11);                     /* changed but not empty */
    esp_rom_printf(rom_p3, safe_mapping_count, safe_copy_count);
    ...
    for (int i = 0; i < safe_mapping_count; i++) {
        ...
        // DIAG: bisect the MMU-loop hang via RAM sentinels (RWDT reboot reports
        // the last one). TODO remove.
        DIAG_PROGRESS(0x20 + i);
        esp_rom_printf(rom_mn, i, (unsigned long)safe_mappings[i].len);
        for (uint32_t e = 0; e < pages; e++) {
            DIAG_PROGRESS(0x40 + i * 16 + e);
            *(volatile uint32_t *)(DR_REG_MMU_TABLE + (first_entry + e) * 4) =
                (paddr_page + e) | SOC_MMU_ACCESS_SPIRAM;
            DIAG_PROGRESS(0x60 + i * 16 + e);
        }
    }
    esp_rom_printf(rom_p4);

    // DIAG: post-MMU marker (flash rodata no longer readable; RAM only). TODO remove.
    DIAG_PROGRESS(6);
    esp_rom_printf(rom_mmu_done);
    ...
    // DIAG: right before the jump. TODO remove.
    DIAG_PROGRESS(7);
    esp_rom_printf(rom_go);

    // DIAG: RTC WDT state at handoff (payload gets reset ~7ms after the jump;
    // suspect a tiny/broken RTC-WDT timeout in the payload's esp_clk_init).
    // Format strings were pre-staged — flash rodata/code is gone post-MMU.
    // TODO remove.
    {
        volatile uint32_t *w = (volatile uint32_t *)0x60008000;
        esp_rom_printf(rom_wdt0, w[0x8C / 4], w[0x90 / 4]);
        esp_rom_printf(rom_wdt1, w[0xA8 / 4], w[0xA0 / 4]);
    }

    // DIAG: read back the entry-point bytes (through the DRAM alias) right
    // before the jump — proves the deferred copy of the payload's .iram0.text
    // actually landed the expected code. TODO remove.
    {
        uint32_t ir = (uint32_t)safe_entry_addr;
        volatile uint32_t *q = (volatile uint32_t *)ir;
        const char *rom_entry = stage + 384;
        esp_rom_printf(rom_entry, ir, q[0], q[1], q[2], q[3]);

        volatile uint32_t *lit = (volatile uint32_t *)0x403745E0;
        const char *f_lit = stage + 448;
        esp_rom_printf(f_lit, lit[0], lit[1], lit[2], lit[3]);
    }
