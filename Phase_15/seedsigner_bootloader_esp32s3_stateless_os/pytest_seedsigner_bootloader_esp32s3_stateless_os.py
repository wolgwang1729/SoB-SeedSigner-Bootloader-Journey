...
#
# Phase 15 port of the Phase 12/13 ESP32-P4 harness. Matches the S3 boot chain
# in main/main.c:
#   1. SD-card / Flash-payload load: mounts FAT32 /sdcard (or dev flash fallback)
#   2. Specter secure app loader (on SD path): bl_section header + platform attr + version
#      check, hash over the main section, secp256k1 multisig verification
#   3. Anti-phishing proof: SHA-256 over the random_fill region, 4 BIP-39 words
#      printed (first boot also provisions the TRNG fill — this test tolerates
...
def test_stateless_bootloader_and_payload(app: IdfApp, dut: IdfDut) -> None:
    ...
    # ====================================================================
    # PHASE 1: LOADER START & PAYLOAD ACQUISITION (SD or Flash Dev Path)
    # ====================================================================
    logging.info('Phase 1: Payload acquisition...')

    load_match = dut.expect([
        r'\[SD CARD\] Loaded (\d+) bytes from /sdcard/seedsigner_esp32s3\.bin',
        r'\[FLASH PARTITION\] Loaded (\d+) bytes from \'payload\''
    ], timeout=15)
    ...
    if 'SD CARD' in _decode(load_match.group(0)):
        ...
        logging.info(f'  ✓ Loaded {loaded_bytes:,} bytes from flash payload partition (dev path)')
        pl_ver = 1
