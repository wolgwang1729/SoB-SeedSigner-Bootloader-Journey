#!/usr/bin/env bash
# ==============================================================================
# Flash MicroPython Payload Script — Phase 16 (dev/bring-up only)
#
# Flashes the RAW MicroPython shim image into the 'payload' data partition at
...
# to bring up the loader -> JMP -> PSRAM execution chain with MicroPython on
# silicon before the S3's SD wiring is soldered. The production path (SD card)
# is untouched.
#
...
# ------------------------------------------------------------------------------
log_step "Flash raw MicroPython payload to 'payload' partition @ 0x220000"
PAYLOAD_BIN="$SCRIPT_DIR/build/micropython_esp32s3_raw.bin"
[ -f "$PAYLOAD_BIN" ] || PAYLOAD_BIN="$HOME/Desktop/seedsigner-micropython-builder/build/WAVESHARE_ESP32_S3_TOUCH_LCD_35B/micropython.bin"

if [ ! -f "$PAYLOAD_BIN" ] || [ ! -s "$PAYLOAD_BIN" ]; then
    log_error "Raw MicroPython payload image not found: $PAYLOAD_BIN"
    log_error "Build it first: $SCRIPT_DIR/build_micropython.sh"
    exit 1
fi
...
log_success "Raw MicroPython payload flashed to 'payload' @ 0x220000."
