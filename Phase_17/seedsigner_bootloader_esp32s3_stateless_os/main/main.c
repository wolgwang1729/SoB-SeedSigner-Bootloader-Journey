...
// --- ESP-IDF: SD card (SDMMC1 native & SDSPI interface on ESP32-S3) ---
...
// Mount the SD card via SDSPI (for HW-125 adapter) or SDMMC (ESP32-S3).
sdmmc_card_t *mount_storage_sdcard(void)
{
    ...
    // Enable internal pull-ups on SPI pins for signal integrity on breadboard
...
static void diag_ps(const char *where)
{
    ...
    ESP_LOGI(TAG, "[DBG] ps @%s: ps=0x%08X wb=%d ws=0x%08X intena=0x%08X",
             where, (unsigned int)_ps, (int)_wb, (unsigned int)_ws, (unsigned int)_ai);
}
...
void dump_cache_err_status(const char *where)
{
    ...
    ESP_LOGI(TAG, "[DBG] cache-err status @%s: ilg=0x%08X acs0=0x%08X acs1=0x%08X dbus_reject_vaddr=0x%08X",
             where, (unsigned int)ilg, (unsigned int)acs0, (unsigned int)acs1, (unsigned int)dbus_reject);
}
...
void app_main(void)
{
    ...
    // ----------------------------------------------------------------
    // Step 1: Load the payload from SD card.
    //   Reads /sdcard/seedsigner_esp32s3.bin — a Specter bundle
    //   [bl_section_t "main"][raw ESP32 image][bl_section_t "sign"].
    //   SD card is unmounted immediately after read for anti-TOCTOU safety.
    // ----------------------------------------------------------------
