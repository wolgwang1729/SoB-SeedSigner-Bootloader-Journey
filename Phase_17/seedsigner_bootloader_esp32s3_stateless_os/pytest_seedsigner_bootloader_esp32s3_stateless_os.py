...
#
# Pytest integration test for the SeedSigner Stateless Bootloader (ESP32-S3) — Phase 17.
#
# Phase 17 Pure SD-Card Boot & Hardened Flash model. Matches the S3 boot chain
# in main/main.c:
#   1. Pure SD-card load: mounts FAT32 /sdcard, reads /sdcard/seedsigner_esp32s3.bin,
#      unmounts immediately (anti-TOCTOU protection). No flash payload partition.
#   2. Specter secure app loader: bl_section header + platform attr ("seedsigner_esp32s3")
#      + version check (pl_ver >= 1), hash over the main section, secp256k1 multisig verification
#   3. Anti-phishing proof: SHA-256 over the expanded 5.875 MB random_fill region,
#      4 BIP-39 words printed (first boot also provisions the TRNG fill)
...
#   6. Bare-metal jump sequence JMP[1..5] (WDT/interrupt teardown, direct
#      copies, shared I/D MMU table reprogramming, cache invalidation)
#   7. Payload boot through the stateless_shim (rug-pull interceptors) —
#      stock hello-world / MicroPython runs from PSRAM
...
# The S3 has ONE shared MMU table used by both the data window (0x3C000000)
# and the instruction window (0x42000000). Entries are indexed by the linear
# (window-masked) address >> 16; page size is 64 KB.
...
# Minimum set of "rug pull" interceptors that must fire for stateless boot to work.
MIN_INTERCEPTOR_COUNT = 1

# Payload reboot cycles (stock hello-world auto-restarts after ~10 s)
STABILITY_CYCLES = 1
...
def test_stateless_bootloader_and_payload(app: IdfApp, dut: IdfDut) -> None:
    ...
    Validates SD-card / flash load + anti-phishing proof + JMP hand-off +
    stock hello-world execution from PSRAM through the stateless shim.
    """
    ...
    segments = []           # list of (addr, len, route) tuples
    mmu_maps = []           # list of (vaddr_linear_entry, pages)
    interceptors_seen = set()  # interceptor names
    ap_words = None         # 4-tuple of anti-phishing words
    ...
    load_match = dut.expect(
        r'\[SD CARD\] Loaded (\d+) bytes from /sdcard/seedsigner_esp32s3\.bin',
        timeout=20
    )
    ...
    # ====================================================================
    # PHASE 6: BARE-METAL JUMP SEQUENCE
    # ====================================================================
    logging.info('Phase 6: Bare-metal jump sequence...')

    dut.expect(r'JMP0 entered|JMP\[1\] entered', timeout=5)
    logging.info('  ✓ JMP[1]: Entered bare-metal jump function (IRAM, JMP zone)')

    dut.expect(r'JMP2 wdts off|JMP\[2\]', timeout=5)
    logging.info('  ✓ JMP[2]: RWDT/MWDT0/MWDT1 disabled, SysTick + timer IRQs silenced')

    dut.expect(r'JMP3 writeback done|JMP\[3\]', timeout=5)
    logging.info('  ✓ JMP[3]: Direct copies into internal SRAM')

    dut.expect(r'JMP5 MMU programmed|JMP\[4\]', timeout=5)
    logging.info('  ✓ JMP: Programming shared I/D MMU table')

    dut.expect(r'JMP6 caches flushed, jumping|JMP\[5\] JUMP!', timeout=5)
    logging.info('  ✓ JMP: Caches flushed, final jump to payload entry point')

    # ====================================================================
    # PHASE 7: PAYLOAD ENTRY & INITIALIZATION
    # ====================================================================
    logging.info('Phase 7: Payload entry & initialization...')

    dut.expect('=== STATELESS SHIM PSRAM PAYLOAD BOOT OK ===', timeout=10)
    dut.expect('=== __wrap_call_start_cpu0 ENTERED ===', timeout=5)
    logging.info('  ✓ Shim entry: __wrap_call_start_cpu0')

    dut.expect('BSS cleared.', timeout=10)
    dut.expect('Clocks initialized.', timeout=10)
    dut.expect('Initializing MMU software contexts...', timeout=10)
    dut.expect('Calling init functions...', timeout=10)
    dut.expect('Running global constructors...', timeout=20)
    dut.expect('Jumping to esp_startup_start_app...', timeout=20)
    ...
    for cycle in range(1, STABILITY_CYCLES + 1):
        ...
        chip_match = dut.expect(
            r'This is esp32s3 chip with (\d+) CPU core\(s\), .*silicon '
        ...
        dut.expect(r'rst:0xc', timeout=5)
    ...
    logging.info('=' * 64)
    logging.info(f'  Payload size:          {loaded_bytes:,} bytes')
    ...
    logging.info(f'  MMU footprint:         {mmu_footprint:,} bytes')
    logging.info(f'  Shim initialization:   Complete (all stages OK ✓)')
    logging.info(f'  Payload stability:     {STABILITY_CYCLES} reboot cycle, no WDT resets')
