#!/usr/bin/env bash
# ==============================================================================
# Build & Flash Loader Script — Phase 17
# Target  : ESP32-S3 Stateless Secure Loader (Pure SD-Card Boot & Hardened Flash)
# Payload : Specter-signed firmware bundle read from the SD card
#           (/sdcard/seedsigner_esp32s3.bin). Build + sign the payload first
#           with ../build_payload.sh (hello-world shim) or ../build_micropython.sh,
#           copy the signed file to a FAT32 SD card root, then run this script.
#           The loader mounts the SD card at boot, reads + verifies the bundle,
#           unmounts immediately (anti-TOCTOU), and executes from PSRAM.
#           This script only builds and flashes the loader artifacts (bootloader,
#           partition table, loader app) — it is payload-agnostic.
# ==============================================================================
set -uo pipefail
...
# Step 1: Build loader
# ------------------------------------------------------------------------------
log_step "Step 1: Build loader (seedsigner_bootloader_esp32s3_stateless_os)"
...
# ESP32-S3 layout: bootloader @0x0, partition table @0x10000, app @0x20000
# (matches partitions.csv).
esptool.py --chip esp32s3 --port "$PORT" --baud "$BAUD" \
    --before default_reset --after hard_reset write_flash \
    --flash_mode dio --flash_freq 80m --flash_size 8MB \
    0x0 "$BOOTLOADER_BIN" \
    0x10000 "$PARTITION_BIN" \
    0x20000 "$LOADER_BIN"

log_success "Flash complete."
log_warn "Reminder: put seedsigner_esp32s3.bin on the SD card root before booting."
