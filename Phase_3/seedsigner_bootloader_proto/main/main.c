...
void app_main(void)
{
    ...
    fw_size = 256 * 1024; // Need enough to read the signature at the end
    ...
    custom_esp_image_header_t header_copy = *img_hdr; // copy it so we can free psram_buf
    ...
    for (int i = 0; i < header_copy.segment_count; i++) {
        ...
        if (seg_hdr.load_addr >= SOC_IROM_LOW && seg_hdr.load_addr < SOC_IROM_HIGH) {
            ...
            if ((seg_hdr.load_addr >= 0x3FC80000 && seg_hdr.load_addr < 0x3FC80000 + 0x80000) || 
                ...
                // Use psram_buf directly since we read the entire firmware into it
                pending_copies[pending_count].dest = (void*)seg_hdr.load_addr;
                pending_copies[pending_count].src = (void*)(main_pl_addr + offset);
    ...
    for (int i = 0; i < pending_count; i++) {
        ESP_LOGI(TAG, "Copy %d: dest=%p, src=%p, len=%lu", i, pending_copies[i].dest, pending_copies[i].src, (unsigned long)pending_copies[i].len);
    }
    
    vTaskDelay(100 / portTICK_PERIOD_MS);
    
    do_mmu_mapping_and_jump(paddr, max_offset, header_copy.entry_addr, pending_copies, pending_count, mmu_mappings, mapping_count);
}
