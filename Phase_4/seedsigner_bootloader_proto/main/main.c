...
static void RTC_IRAM_ATTR __attribute__((noreturn)) do_mmu_mapping_and_jump(void) {
    ...
    for (int i = 0; i < safe_mapping_count; i++) {
        ...
        uint32_t pages = safe_mappings[i].len >> 16;
    ...
    // Invalidate caches using ROM functions
...
void app_main(void)
{
    ...
    esp_flash_read(NULL, &qemu_main_hdr, QEMU_FLASH_PADDR, sizeof(qemu_main_hdr));
    ...
    esp_flash_read(NULL, &header_copy, QEMU_FLASH_PADDR + payload_offset, sizeof(header_copy));
#else
    ...
    for (int i = 0; i < header_copy.segment_count; i++) {
        ...
        if (seg_hdr.load_addr >= SOC_IROM_LOW && seg_hdr.load_addr < SOC_IROM_HIGH) {
            ...
            esp_flash_read(NULL, safe_dram_ptr, paddr + offset, seg_hdr.data_len);
